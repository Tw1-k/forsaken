...
Toggles.MC_InfiniteStamina:OnChanged(function(value)
    ...
    if value then
        ...
        Library:Notify({Title = "Infinite Stamina Enabled", Content = "You now have unlimited stamina", Duration = 3})
    else
        ...
        Library:Notify({Title = "Infinite Stamina Disabled", Content = "Stamina consumption is now normal", Duration = 3})
    end
end)
...
Toggles.MG_AutoComplete:OnChanged(function(value)
    ...
    if value then
        ...
        Library:Notify({Title = "Auto-Complete Enabled", Content = "Generators will complete automatically", Duration = 3})
    else
        if puzzleuiconnection then puzzleuiconnection:Disconnect() puzzleuiconnection = nil end
        Library:Notify({Title = "Auto-Complete Disabled", Content = "Manual generator completion required", Duration = 3})
    end
end)
...
Toggles.VE_EnableESP:OnChanged(function(value)
    ...
    Library:Notify({Title = "ESP " .. (value and "Enabled" or "Disabled"), Content = "Visual indicators are now " .. (value and "visible" or "hidden"), Duration = 3})
end)
...
Toggles.VF_CustomFOV:OnChanged(function(value)
    ...
    Library:Notify({Title = "Custom FOV " .. (value and "Enabled" or "Disabled"), Content = value and "FOV controls active" or "FOV reset to default", Duration = 3})
end)
